...
const PlanCard = ({ plan, actionText, action }: IPlanCardProps) => {
  return (
    <Paper w="100%" h='100%' mx='3.2%' withBorder shadow="md" radius="md" display='flex' style={{ flexDirection: 'column', borderRadius: '17px' }} /* className='rounded-2xl' */>
      <Box w='350px' h='211px' bg={{ base: 'primary'}} style={{borderRadius: '17px'}}/* className='rounded-2xl' */>
        <Text ta="center" tt='uppercase' c={'white'} fw={700} fz="lg" style={{marginTop:'44px', marginBottom:'20px'}}>{plan.planName}</Text>
        <Text mx='31px' w='288px' h='55px' ta="center" c='primary' size='lg' bg='white' style={{ borderRadius: '10px',
          alignContent: 'center',
          display: 'flex',
        ...
        <Text ta='center' c='white'>{plan.description}</Text>
      </Box>
